...
*Data repo*s contain just one type of configuration file (Table 2): (todo: auto table & section references)
...
CLI commands create templates of some of these files. (todo: which commands & files?)
...
A data-schema migration configuration can be fully initialized by a CLI command. (todo: which one?)
...
(todo: a diagram of the data flow through migration might help)
