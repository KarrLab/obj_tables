...
class SchemaChanges(object):
    ...
    # todo: test & use
    @staticmethod
    def validate(schema_changes_args):
        """ Check that the attributes of the arguments to `SchemaChanges` have the right structure

        Args:
            schema_changes_args (:obj:`dict`): arguments to `SchemaChanges` generated by loading a schema
                changes file

        Returns:
            :obj:`list`: errors in `schema_changes_args`, if any
        """
        errors = []
        for str_attr in ['commit_hash', 'transformations_file']:
            if not isinstance(schema_changes_args[str_attr], str):
                errors.append("schema_changes_args[{}] must be a str, but is a {}".format(str_attr, type(str_attr)))

        required_structure = "'renamed_models' must be None, or a list of pairs of strings"
        try:
            if schema_changes_args['renamed_models'] is not None:
                for renaming in schema_changes_args['renamed_models']:
                    for pair in renaming:
                        if len(pair) != 2 or not isinstance(pair[0], str) or not isinstance(pair[1], str):
                            errors.append(required_structure)
        except TypeError as e:
        ...
        required_structure = "'renamed_attributes' must be None, or a list of pairs of pairs of strings"
        try:
            if schema_changes_args['renamed_attributes'] is not None:
                for renaming in schema_changes_args['renamed_attributes']:
                    ...
                    for attr_name in existing, migrated:
                        if len(attr_name) != 2 or not isinstance(attr_name[0], str) or \
                            not isinstance(attr_name[1], str):
                            errors.append(required_structure)
                            continue
        except TypeError as e:
            errors.append(required_structure + ", but examining it raises a '{}' error".format(str(e)))
