# commit 759758bcd5e5b8829ea8f61e4d3d054060eb7219 of wc_lang
...
class ObjectiveFunction(obj_model.Model):
    ...
    class Meta(obj_model.Model.Meta):
        ...
        valid_used_models = ('ParameterRenamed', 'Observable', 'Reaction', 'BiomassReaction')
...
class FunctionExpression(obj_model.Model):
    ...
    parameters = ManyToManyAttribute('ParameterRenamed', related_name='functions')
    ...
    class Meta(obj_model.Model.Meta):
        ...
        valid_used_models = ('ParameterRenamed', 'Observable', 'Function')
...
class StopConditionExpression(obj_model.Model):
    ...
    parameters = OneToManyAttribute('ParameterRenamed', related_name='stop_conditions')
...
class RateLawEquation(obj_model.Model):
    ...
    parameters = ManyToManyAttribute('ParameterRenamed', related_name='rate_law_equations')
    ...
    class Meta(obj_model.Model.Meta):
        ...
        ordering = ('rate_law',)
        valid_functions = (ceil, floor, exp, pow, log, log10, min, max)
        valid_used_models = ('Species', 'ParameterRenamed', 'Observable', 'Function')
    ...
    def deserialize(cls, attribute, value, objects):
        ...
        parameter_pattern = r'(^|[^a-z0-9_\[\]])({})([^a-z0-9_\[\]]|$)'.format(ParameterRenamed.id.pattern[1:-1])
        ...
        try:
            ...
            for match in re.findall(parameter_pattern, value, flags=re.I):
                if match[1] not in reserved_names:
                    parameter, error = ParameterRenamed.deserialize(match[1], objects)
                    if error:
...
class ParameterRenamed(obj_model.Model):
    ...
    Related attributes:
        functions (:obj:`list` of :obj:`FunctionExpression`): FunctionExpressions that use a ParameterRenamed
    """
    ...
    def add_to_sbml_doc(self, sbml_document):
        """ Add this ParameterRenamed to a libsbml SBML document.

        Args:
             sbml_document (:obj:`obj`): a `libsbml` SBMLDocument

        Returns:
            :obj:`libsbml.ParameterRenamed`: the libsbml ParameterRenamed that's created
