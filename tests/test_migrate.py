...
# todo: move ExpressionMethods to obj_model, and make obj_model local expression models to test
from wc_lang.core import ExpressionMethods
...
class MigrationFixtures(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.no_change_migrator_model = self.set_up_expression_fixture(
            self.wc_lang_no_change_migrator, 'Parameter', 'Parameter')
        self.changes_migrator_model = \
            self.set_up_expression_fixture(self.wc_lang_changes_migrator, 'Parameter', 'ParameterRenamed')

    def set_up_expression_fixture(self, migrator, existing_param_class, migrated_param_class):
        migrator.initialize()
        migrator.prepare()
        ...
        # todo: test without Observable in objects; what traps the WcLangExpressionError?
        model = Model(id='test_model', version='0.0.0')
        param = model.parameters.create(id='param_1')
        objects[ParameterClass]['param_1'] = param
        fun_1 = ExpressionMethods.make_obj(model, Function, 'fun_1', 'log10(10)', objects)
        objects[Function]['fun_1'] = fun_1
        ExpressionMethods.make_obj(model, Function, 'fun_2', 'param_1 + 2* Function.fun_1()', objects)
        ExpressionMethods.make_obj(model, Function, 'disambiguated_fun', 'Parameter.param_1 + 2* Function.fun_1()',
            objects)
        return model
        ...
        for migrator, model in zip(migrators, models):
            for fun in model.functions:
                if migrator == self.wc_lang_changes_migrator and fun.id == 'disambiguated_fun':
                    ...
                    wc_lang_expr = fun.expression.analyzed_expr
        ...
        for migrator, model in zip(migrators, models):
            ...
            for existing_model, migrated_model in zip(all_existing_models, migrated_models):
                ...
                if existing_model in existing_non_expr_models:
                    self.assertFalse(hasattr(migrated_model, Migrator.ANALYZED_EXPR))
                else:
                    self.assertTrue(hasattr(migrated_model, Migrator.ANALYZED_EXPR))
                    existing_expr = getattr(existing_model, Migrator.ANALYZED_EXPR)
                    migrated_expr = getattr(migrated_model, Migrator.ANALYZED_EXPR)
            ...
            if migrator == self.wc_lang_no_change_migrator:
                ...
                last_existing_fun_expr.analyzed_expr.expression = \
                    last_existing_fun_expr.analyzed_expr.expression + ':'
...
class TestMigrationController(MigrationFixtures):
    ...
    @unittest.skip('skipping until works')
    def test_wc_lang_migration(self):
        ...
        cProfile.runctx('migrated_filename = MigrationController.migrate_over_schema_sequence(migration_desc)',
        ...
        # validate
        existing = read_workbook(wc_lang_model_copy)
